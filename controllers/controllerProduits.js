...
module.exports = {
    ...
    getAllLegume: (req, res) => {
        ...
        try {
            ModelAvis.getAllLegume(siret_producteurs, (err, result) => {
                if (err) {
    ...
    getAllFruit: (req, res) => {
        ...
        try {
            ModelAvis.getAllFruit(siret_producteurs, (err, result) => {
                if (err) {
                    console.error("Erreur lors de la récupération des avis :", err);
                    return res.status(500).json({ error: "Erreur lors de la récupération des fruits." });
                }
    
                if (!result || result.length === 0) {
                    return res.status(404).json({ message: "Aucun fruit trouvé pour ce producteur." });
                }
    ...
    */
    ...
    registerAvis: (req, res) => {
        ...
        try {
            ModelAvis.registerProduits(siret_producteurs, id_consommateurs, note_avis, date_avis, message_avis, (err, result) => {
                if (err) {
    ...
    */
    ...
    deleteAvis: (req, res) => {
        let {id_avis} =  req.body
        console.log(req)
        //let siret_token = req.user.id
        //console.log(siret_token)
        try {
            ModelAvis.deleteAvis(id_avis, (err, result) => {
                if (err) {
                    return res.status(500).json({ error: 'Erreur lors de la suppression de l\'avis.'+err+'' });
                }
                res.status(201).json({ message: 'Avis supprimé avec succès.' });
            });
        } catch (error) {
            res.status(500).json({ error: 'Erreur interne.' });
        }
